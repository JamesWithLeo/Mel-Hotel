...
export default function PendingBookingTable({ data }: { data: IBookSlice[] }) {
  ...
  const table = useMaterialReactTable({
    ...
    renderRowActionMenuItems: ({ row }) => {
      const handleDelete = async () => {
        if (
          window.confirm(
            "Are you sure want to delete this pending reservation? ",
          )
        ) {
          await axios
            .delete(`/melhotel/collection/`, {
              params: { id: row.original._id, collection: "pending" },
            })
            .then(async (respone) => {
              console.log(respone);
            })
